...
the host library's inference). `external_payload` flags wrapped opaque callables as a preservation
risk (plan A.3.1).
"""
...
def _form_from_meta(result: object, leading_none: bool) -> NumpyForm:
...
class NumpyBackend:
    ...
    def op_form(self, op: str, inputs: Sequence[object], params: Mapping[str, object]) -> NumpyForm:
        ...
        result = self._apply(op, [_meta(f) for f in forms], params)
        leading_none = any(f.shape and f.shape[0] is None for f in forms)
        return _form_from_meta(result, leading_none)
